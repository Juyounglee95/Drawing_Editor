...
public class EditorSquare extends EditorShape {
	
	private static final int TYPE = Editor.SQUARE;
	
	public EditorSquare(String sid,int x, int y, int w, int h, Color color) {
		this.sid = sid;
		this.x = x;
		this.y = y;
		this.width = w;
	...
	//	System.out.println(width);
		g.setColor(color);
		//System.out.println("sqare color "+ color);
		
		g.fillRect(x, y, Math.min(width,  height), Math.min(width,  height));
		
		g.drawRect(x, y, Math.min(width,  height), Math.min(width,  height));
